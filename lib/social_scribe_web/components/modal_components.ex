defmodule SocialScribeWeb.ModalComponents do
  ...
  attr :class, :string, default: nil
  attr :mapping_event, :string, default: nil
      ...
      <div id={@details_id} class="mt-2 pl-8">
        ...
        <div class="mt-3 grid grid-cols-[1fr_32px_1fr] items-start gap-6">
          <%= if @mapping_event do %>
            <button
              type="button"
              phx-click={@mapping_event}
              phx-value-field={Map.get(@suggestion, :source_field) || @suggestion.field}
          ...
          <% else %>
            <button
              type="button"
              disabled
              class="text-xs text-slate-400 font-medium justify-self-start cursor-not-allowed"
            >
              Update mapping
            </button>
          <% end %>
